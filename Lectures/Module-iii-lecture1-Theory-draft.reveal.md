...
<--o-->
<!-- PROPOSED SLIDE 2026-08-25 — flagged, not written; needs a figure that does not exist yet. -->
## &#10133; Proposed &mdash; Stress trajectories

<small><em>Placeholder for structural review. Nothing below is written yet.</em></small>

<small>

A single slide on **stress trajectories**, using the southern-California model the
deck already relies on, so the concept lands on a familiar picture rather than an
abstract one:

- curves everywhere tangent to a principal stress direction &mdash; the field-scale
  reading of the principal directions introduced on the *Geometry of the stress
  tensor* slide
- drawn on the same Big Bend geometry as the &Delta;CFF map, so the restraining
  bend's swing in orientation is visible as bending trajectories
- **a few stress crosses overlaid** at anchor points, tying each trajectory back to
  the $\sigma_1/\sigma_3$ pair it is drawn from
- reads directly onto the free-surface argument above: one principal axis must be
  vertical at the surface, so the trajectories are horizontal projections

*Figure needed:* new Underworld3 output &mdash; trajectory integration through the
principal-direction field of the existing `california` model, plus crosses at
selected points. Not a re-style of a cached figure; needs a run.
