...
1. **Calculate extension from elongation:**
   ...
   - e = (l - l~0~)/l~0~ = (5-3)/3 = 0.67 (67% extension)

2. **Calculate beta factor:**
   - β = l/l~0~ = 5/3 = 1.67

3. **Use Mohr circle to find stress on a plane:**
   - Given σ~1~ = 100 MPa, σ~3~ = 20 MPa
   - Find normal and shear stress on plane at 30° to σ~1~

4. **Predict fault angle from Anderson's theory:**
   - Given friction coefficient μ = 0.6
   - tan 2θ = ∓1/μ = ∓1.67
   - θ ≈ 30° to σ~1~

5. **Calculate effective stress:**
   - Given total stress σ~n~ = 100 MPa, pore pressure P~f~ = 40 MPa
   - σ'~n~ = σ~n~ - P~f~ = 60 MPa

6. **Apply Mohr-Coulomb to predict failure:**
   - τ = C + σ~n~·tan φ
   - Given C = 10 MPa, φ = 30°, σ~n~ = 50 MPa
   - τ = 10 + 50·tan(30°) ≈ 39 MPa

7. **Use Byerlee's Law:**
   - At shallow depth (<2 km): τ = 0.85σ~n~
   - At greater depth: τ = 50 + 0.6σ~n~
